...
RSpec.describe 'Advanced Nested Collections' do
  ...
  it 'test 7' do
    ...
    # all restaurants
    employees = stores[:olive_garden][:employees] << stores[:dennys][:employees] << stores[:macdonalds][:employees]
  ...
  it 'test 8' do
    ...
    ingredients_all = stores[:olive_garden][:dishes].map { |dish| dish[:ingredients] } << stores[:dennys][:dishes].map { |dish| dish[:ingredients] } << stores[:macdonalds][:dishes].map { |dish| dish[:ingredients] }
    ingredients = ingredients_all.flatten
    expected = [
      "Rice",
      "Cheese",
      "Butter",
  ...
  it 'test 11' do
    ...
    olive_garden_menu = stores[:olive_garden][:dishes].map { |dish| [dish[:name], dish] }.to_h
    dennys_menu = stores[:dennys][:dishes].map { |dish| [dish[:name], dish] }.to_h
    macdonalds_menu = stores[:macdonalds][:dishes].map { |dish| [dish[:name], dish] }.to_h
